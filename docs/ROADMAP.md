...
- **`docker compose up` now works** (2026-06-26). Both images build; `atlas` + `app` come up healthy and the control room serves — `GET /` (SPA) 200, `GET /api/personas` returns the seeded library, `GET /api/site/targets` 200 (Mongo wired). The vector-index init step (punch-list #2) is still outstanding, so `$vectorSearch` retrieval isn't exercised yet, but the stack boots and serves.
...
2. **Atlas vector-index init** — atlas-local serves `$vectorSearch`, but the `site_surfaces`/`site_knowledge` vector indexes (384-d) need a one-shot creation step on boot. ← **next up**
